...
mkRandObjects = do
  brickList <- mapM (const getRandPos) [1..70]
  let ob = map (`Object` Brick) brickList
...
updateStateHelper keyInput env@(Env column row boardSize) state@(GameState boardPos directionY directionX gameStatus objectsMap collisions score debug)
 ...
 where
  ...
  newObjectsMap = destroyBricks newCollisions objectsMap
  ...
  newObjects = Map.adjust (const newBall) "ball" newObjectsMap
...
destroyBricks :: Ord k => [k] -> Map.Map k a -> Map.Map k a
destroyBricks xs obMap = foldl (flip Map.delete) obMap xs
